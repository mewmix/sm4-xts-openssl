...
int main(int argc, char *argv[]) {
    if (argc != 5) {
        fprintf(stderr, "Usage: %s <-e|-d> <AES_key> <IV> <input_text_or_ciphertext>\n", argv[0]);
        return 1;
    }

    /* Extract command-line arguments */
    const char *operation = argv[1];
    unsigned char binary_key[64]; // 512-bit key for AES-256-XTS
    unsigned char binary_iv[16];  // 128-bit IV

    // Convert hex key and IV to binary
    hex_to_bin(argv[2], binary_key, sizeof(binary_key)); // Ensure argv[2] has 128 hex characters
    hex_to_bin(argv[3], binary_iv, sizeof(binary_iv));   // Ensure argv[3] has 32 hex characters

    // Print the binary AES key and IV for verification
    print_hex(binary_key, sizeof(binary_key), "AES Key (binary)");
